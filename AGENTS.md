...
- README 운영은 이중 언어를 선호한다: `README.md`는 영어 전용으로 유지하고 한국어 문서는 `README.ko.md`로 분리한다.
- GitHub 저장소 운영 작업(라벨·마일스톤·프로젝트·PR 생성/머지·레포 메타데이터 수정)은 에이전트가 직접 실행해 주는 방식을 선호한다.
...
- GitHub 운영 기준 세트가 이미 적용돼 있다: `type/area/priority/status/sdd` 라벨 체계, `M1/M2/M3` 마일스톤, 그리고 `RE Tension Game Making - Delivery Board` 프로젝트 보드가 생성되어 있다.
- 회전 중 멈춤 이슈는 입력장치 종속이 아니며(맥 트랙패드뿐 아니라 윈도우 마우스·`J/L` 회전에서도 재현), 각도 의존 레이캐스트 수치 불안정 가능성이 주요 원인 후보로 기록돼 있다.
